...
El Cálculo Lambda, introducido por Alonzo Church en los años 1930, es un sistema formal que permite construir expresiones mediante dos operaciones básicas: la abstracción y la aplicación \cite{pierce2002types}. En este sistema, algunas expresiones representan funciones (usando la notación lambda) y otras representan valores sobre los que pueden aplicarse dichas funciones. Dos expresiones pueden yuxtaponerse para formar una nueva expresión; si la primera es una abstracción, entonces se interpreta como aplicación funcional. A partir de ahí, pueden realizarse reducciones para simplificar la expresión resultante si corresponde.
...
En Lean, definimos funciones utilizando el comando \bluecode{fun}, que corresponde a la notación $\lambda$ del cálculo lambda clásico\footnote{En la versión anterior de Lean, se utilizaba la notación \code{$\lambda$ n, n + 2}, sin embargo en esta última versión se ha cambiado a \code{fun n $\mapsto$ n + 2} para mejorar la legibilidad del código.}. Por ejemplo\footnote{Estudiaremos el comando \bluecode{def} en detalle más adelante.}:
...
Por ejemplo, en Lean, el tipo \code{List $\alpha$} representa una lista de elementos de tipo $\alpha$. Si definimos un objeto de tipo \code{List $\alpha$}, el tipo de este objeto dependerá del tipo $\alpha$ que le asignemos Internamente, se define como una función de tipo \code{Type u $\to$ Type u}.
...
Podemos pensar en los tipos dependientes como una generalización de las funciones que van de un tipo en otro tipo, y el tipo de llegada puede depender del tipo de entrada. Llamamos \quotes{funciones dependientes} a este tipo de funciones.

En particular, la proposición $\forall x \in \nat, P x$ se representa en Lean mediante un tipo dependiente que expresa el conjunto de las funciones que, dado un elemento $x \in \nat$, devuelve una prueba de $P x$.  La notación de Lean para este tipo de construcción es \code{$\Pi$ x : $\nat$, P x}. El operador $\Pi$ sirve para denotar que el tipo de salida puede depender del valor de entrada.
