...
En Lean, los tipos tienen su propio tipo, que recibe el nombre de \blue{}code{Type}. Esto nos permite definir nuevos tipos. Podemos utilizar el comando \bluecode{variable} para definir objetos en nuestro código\footnote{Veremos este comando en detalle más adelante}.
...
En un caso más general, si \code{$\beta$ : $\alpha$ $\to$ Type} es una familia de tipos indexada por $\alpha$, entonces una función de tipo \code{f : $\alpha$ $\to$ $\beta$ $\alpha$} es una función dependiente, ya que el tipo de salida de $f$ depende del argumento de entrada.

Una forma importante en la que aparecen los tipos dependientes es en las proposiciones cuantificadas. Por ejemplo, la proposición $\forall x \in \nat, P x$ se representa en Lean como un tipo dependiente: el tipo de las funciones que, dado un $x$, devuelven una prueba de $P x$. La notación de Lean para este tipo de \quotes{funciones generalizadas} es \code{$\Pi$ x, P x}.
...
Esta propiedad tiene consecuencias importantes. Por un lado, evita comportamientos no deseados cuando definimos estructuras que dependen de proposiciones. Por ejemplo, si quisiéramos definir un punto del primer cuadrante en $\mathbb{R}^2$ como un par de la forma \code{x y : $\mathbb{R} \times \mathbb{R}$} junto con una demostración \code{h : x $\geq$ 0 $\land$ y $\geq$ 0}, entonces gracias a la irrelevancia de las demostraciones podemos identificar dos puntos que tengan las mismas coordenadas, aunque las pruebas \code{h} y \code{h'} asociadas a cada uno sean distintos.

Por otro lado, esta misma propiedad impide acceder al contenido de una prueba. En particular, no es posible extraer directamente el testigo de la demostración de una proposición existencial del tipo $\exists x, P(x)$, ya que todas las demostraciones de esa proposición se consideran iguales. Dada una demostración de $\exists x, P(x)$, para obtener un valor concreto que satisfaga $P$, necesitaremos utilizar el axioma de elección, que en Lean se introduce mediante la función \code{Classical.choose}, como veremos más adelante.
